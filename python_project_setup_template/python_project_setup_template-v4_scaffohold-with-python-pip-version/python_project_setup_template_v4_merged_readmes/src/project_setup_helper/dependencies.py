...
def install_dependencies(config: ProjectConfig) -> None:
    ...
    if ASK_TO_UPGRADE_PIP and ask_yes_no("Upgrade pip inside the virtual environment first?", default=True):
        pip_target = f"pip=={config.pip_version}" if config.pip_version else "pip"
        result = run_command([str(venv_python), "-m", "pip", "install", "--upgrade", pip_target], cwd=config.project_root)
