...
from project_setup_helper.python_versions import (
    ask_pip_version,
...
def print_intro() -> None:
    ...
    print("  Step 2: Project details")
    print("  Step 3: Choose destination")
    print("  Step 4: Review summary")
    print("  Step 5: Generate project")
    print("\nDetected machine:")
...
def choose_project_name() -> tuple[str, str]:
    """Ask for project name and derive import package name."""
    raw_name = ask_text("\nProject name", default="my-python-project")
...
def review_configuration(config: ProjectConfig) -> None:
    ...
    print(f"  Python req:   >= {config.minimum_python_version}")
    print(f"  Interpreter:  {config.python_interpreter}")
    print(f"  Pip version:  {config.pip_version or 'latest'}")
...
def main() -> None:
    ...
    try:
        ...
        logging.info("Minimum Python version: >=%s", config.minimum_python_version)
        logging.info("Python interpreter: %s", config.python_interpreter)
        logging.info("Pip version: %s", config.pip_version or "latest")
